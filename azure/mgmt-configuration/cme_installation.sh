...
sys.exit(0)
EOF
    cme_bundle_count_in_autoupdater=$($PYTHON3 $CME_AUTOUPDATER_BUNDLE_COUNT_SCRIPT "$AUTOUPDATER_COMMAND_OUTPUT")
    if [ $? -ne 0 ]; then
...
sys.exit(0)
EOF
    cme_curr_verion=$($PYTHON3 $CME_AUTOUPDATER_VERSION_SCRIPT "$AUTOUPDATER_COMMAND_OUTPUT")
    if [ $? -ne 0 ]; then
...
sys.exit(1)
EOF
    cme_status=$($PYTHON3 $CME_INSTALLATION_STATUS_SCRIPT "$AUTOUPDATER_COMMAND_OUTPUT")
    if [ $? -ne 0 ]; then
        LOG_ECHO "Failed to execute Python script for CME version in AutoUpdater"
