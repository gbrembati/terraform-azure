...
data "azurerm_subnet" "frontend" {
  name = azurerm_subnet.net-north-frontend.name
  virtual_network_name = azurerm_virtual_network.vnet-north.name
  resource_group_name = azurerm_virtual_network.vnet-north.resource_group_name
}

data "azurerm_subnet" "backend" {
  name = azurerm_subnet.net-north-backend.name
  virtual_network_name = azurerm_virtual_network.vnet-north.name
  resource_group_name = azurerm_virtual_network.vnet-north.resource_group_name
}
...
resource "azurerm_virtual_machine" "vm-instance-availability-set" {
  ...
  os_profile {
    ...
    custom_data = templatefile("${path.module}/cloud-init.sh", {
      ...
      virtual_network = azurerm_virtual_network.vnet-north.name
